...
| thread              | Calculates a (TM, TN) threadtile of the C blocktile via a mamtul of (TM, BK) @ (BK, TN) threadtiles from A and B respectively. The matmul is done via summing outer product of (TM, 1) slices from A and (1, TN) slices from B |
...
3. Each thread does one iteration of <span style={{color: "oklch(0.62 0.20 145)"}}>outer product</span> accumulation into it's (TM, TN) <span style={{color: "oklch(0.68 0.19 230)"}}>threadtile</span>.
...
Fundamantally a Layout is just a function that maps an ND logical coordinate to a 1D offset in a buffer ie. `(0, 1) -> 1`.
...
Simon's kernel achieves this by advancing the A/B/C pointers via a manual offset calculation to the start of the first blocktile the block need to read (and then incrementing the points in the loop to get the next blocktiles).
...
1. Calculate which parts of the smem blocktiles the thread needs to read in order to calculate it's threadtile.
...
But in the snippet we pass `LayoutRight{}` so the mapping becomes row major, that way consecutive thread will load the same row from A and consecutive colulmns from B leading to coalesced gmem access.
...
Additionally we grab the tile of gC the thread should write it's results to.
...
3. acculumate one outer product into `thread_results` in rmem
...
- Loop bounds are determined by the shape of tensors. For example, the outer loop `dot_idx` ranges from `0..shape<2>(gA)`. 
...
So the outer loop iterates through the blocktile index as we expect. Since `gA_to_r` is just a local_partition of `gA`, it also 3 dimensions and we index into the last one to grab the correct blocktile.

Simon's kernel achieves this keeping points to the start of the right tile, and advancing the pointers throughout the loop.
...
The main takeaway here is that CuTe gives us a powerful language to describe shapes, layouts, and access patterns along with an ergenomic set of utilies to extract a given thread's workload from that description. Fundamentally, that's all we need to write matmuls.