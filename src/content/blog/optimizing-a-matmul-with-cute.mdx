...
With CuTe, we can use these objects to extract the portions of our inputs and outputs and each thread will read and write to. You will see that overarching theme again and again: CuTe gives us a powerful language to describe shapes, layouts, and access patterns along with an ergenomic set of utilies to extract a given thread's workload from that description. Fundamentally, that's all we really need to write matmuls.