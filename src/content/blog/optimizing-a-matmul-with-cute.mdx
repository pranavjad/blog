...
```
This is a simple example but we will see with more complex indexing, having the Layout instead of calculating offsets manually is very nice.
...
1. Calculate which parts of the smem blocktiles the thread needs to read in order to calculate its threadtile.
2. Calculate which parts of global memory this thread should write it's results to.
   In Simon's kernel we also do this, it just shows up as an indexing calculation later on in the hot loop, like `sA[calculation]`.
...
2. Load threadtile slices from A/B from smem -> rmem.
3. Accumulate one outer product into `thread_results` in rmem.
4. Repeat 2-3 BK times.
