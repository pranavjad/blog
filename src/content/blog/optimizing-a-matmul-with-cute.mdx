...
For our purposes we can problem dimensions and tile dimensions concrete.
...
*mouse over the visualization to see how different different coordinates index the tiling*
<LocalTileViz />
...
<Steps start={3}>
  ...
  <Step n={4}>Calculate which parts of global memory this thread should write it's results to. In Simon's kernel we also do this, it just shows up as an indexing calculation later on in the hot loop, like `sA[calculation]`.</Step>
</Steps>
</StepGroup>
...
which would expand to the code above, and even do things like vectorize loads if possible (more on this in a future post).
...
Instead of indexing directly into `C`, we index into the local tile of `C` that this thread got from the local_tile that produced `gC_to_w`. Recalling how `gC_to_w` was defined we can see how it is ultimately equivalent to the indexing expression from Simon's kernel.
