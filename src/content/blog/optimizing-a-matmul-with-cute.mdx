...
Tensor sB = make_tensor(make_smem_ptr(B_smem), B_smem_layout);
```
First we declare the layout of our IO tensors with the correct shape and `LayoutRight{}` which denotes row major in cute (`make_stride(K, 1)` would be equivalent for A for example).
...
Since a layout is just a function, its codomain size is equivalent to the size of a buffer such that any offset that the layout could possibly produce will never out-of-bounds that buffer.
...
Simon's kernel achieves this by advancing the A/B/C pointers to first blocktile the block need to read (and then incrementing the points in the loop).
...
This is the CuTe idiom, declare useful layouts on data and use them to pull this thread/block/warp's workload with the handy utilities.
...
Formally, `local_tile` results in some data shaped like this: `((tile_w, tile_h), (rest_w, rest_h))` and the coord indexes into the second "rest" mode, TODO: add vis).
...
In CuTe we actually do a little more up front work rather than doing complex indexing later which will become clear as we see the main loops of each kernel.
...
But we pass `LayoutRight{}` so the mapping becomes row major, that way consecutive thread will load the same row from A and consecutive cols from B leading to coalesced gmem loads.
